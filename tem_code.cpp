...
class cartItem: public Product{
    ...
    // the classes that inherit this class, will include the quantity property
    int quantity;
...
class shoppingCart{
    ...
    void changeQuantity(_Rb_tree_const_iterator<cartItem> itemIT, cartItem changedItem){
        ...
        changedItem.setterQuantity(prevQuantity+changedItem.getterQuantity());
        ...
        if(changedItem.getterQuantity()){
            totalCost+=(changedItem.getterPrice()*changedItem.getterQuantity()); // calculating the new cost
